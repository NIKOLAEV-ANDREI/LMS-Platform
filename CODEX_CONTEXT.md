...
- Added teacher nav button �������� ����� in layout (rontend/src/app/components/Layout.tsx).
...
- Removed redundant ������� button from unified teacher reviews page (left only �����).
...
## 118) 2026-05-10 - Reduced duplicated frontend API requests
- Root cause:
  - `getSession()` always called `/dashboard` via `syncUserFromDashboard`, causing extra network calls.
  - `getCourses()` called `getSession()` every time, creating cascaded duplicate requests on pages that already called `getSession()`.
- Changes in `frontend/src/app/utils/api.ts`:
  - `getSession(options?: { syncDashboard?: boolean })` now syncs dashboard only when explicitly requested.
  - default `getSession()` path now uses only `/me` and stores mapped user without extra `/dashboard` call.
  - `getCourses()` now reads role from local cached user first and calls `getSession()` only if role is missing.
- Result:
  - fewer duplicate backend requests on dashboard/profile/course pages.
  - no behavior change for sign-in flow where dashboard sync is still used.
- Validation:
  - frontend: `npm run check:encoding` - OK
  - frontend: `npm run build` - OK

## 119) 2026-05-10 - Email regex validation
- Added strict email format validation in backend service layer.
- File: `backend/internal/service/field_limits.go`
  - introduced compiled `emailRegex`
  - `validateEmail` now trims input, checks required/max length, then regex format.
  - returns `invalid email format` on mismatch.
- Added frontend error localization:
  - `invalid email format` -> `???????????? ?????? email`.
- File: `frontend/src/app/utils/api.ts`
- Validation:
  - backend: `go test ./...` - OK
  - frontend: `npm run check:encoding` - OK

## 120) 2026-05-10 - Detailed frontend error localization
- Improved error messages in `frontend/src/app/utils/api.ts` to avoid generic "????????? ?????? ?? ???????".
- Added detailed parser for backend validation errors:
  - `<field> is required`
  - `<field> is too long (max N chars)`
  - `<field> is too short (min N chars)`
  - attachment limits/format errors
  - question validation errors for tests
  - `forbidden: ...` reasons
- Added field-name translation map (name/email/password/module/lesson/video/etc.) for clearer Russian messages.
- Unknown English backend error now returns `?????? ???????: <raw message>` instead of generic fallback.
- Validation:
  - frontend: `npm run check:encoding` - OK
  - frontend: `npm run build` - OK

## 121) 2026-05-10 - Expanded user-facing error messages
- Extended frontend error localization in `frontend/src/app/utils/api.ts`.
- Added missing translations for frequent backend/runtime errors:
  - token expired / invalid claims
  - not implemented
  - duplicate key / foreign key / relation missing / DB auth / connection refused / timeout
  - forbidden teacher ownership case
- Added detailed parser rules for:
  - unique-constraint names (`users_email_key`, `users_public_id_idx`, `courses_public_id_idx`, lesson submission uniqueness)
  - foreign-key violations
- Kept existing field-level validator message parser (`required`, `too long`, `too short`) and improved fallback:
  - unknown English server errors now shown as `?????? ???????: <raw message>`.
- Validation:
  - frontend: `npm run check:encoding` - OK
  - frontend: `npm run build` - OK

## 122) 2026-05-11 - Server-side test engine: settings, random pool, attempts history, analytics
- Backend (`Go`):
  - Extended test model in `backend/internal/domain/models.go`:
    - `LessonTest.settings`: time limit, pass score, max attempts, random question count, shuffle questions, shuffle options, show correct answers.
    - `LessonQuestion`: added `correctText`, `difficulty`, and support for `true_false` type.
  - Added DB migration in `backend/internal/db/migrate.go`:
    - new table `lesson_test_attempts` for persisted attempts, scores, answers, and per-attempt snapshots.
  - Added repository API methods in `backend/internal/repository/interfaces.go` and implemented them in `backend/internal/repository/postgres/enrollment_repo.go` (plus sqlite stubs).
  - Added full service flow in `backend/internal/service/course_service.go`:
    - start attempt with random question sampling (e.g. 10 from 40) and optional shuffling.
    - submit attempt with server-side scoring and pass/fail decision.
    - store attempts history with score.
    - teacher/admin analytics: who passed, who failed, attempts used, question difficulty stats.
  - Added HTTP endpoints in `backend/internal/handler/http/handler.go`:
    - `POST /api/courses/{courseID}/lessons/{lessonID}/test/start`
    - `POST /api/courses/{courseID}/lessons/{lessonID}/test/submit`
    - `GET /api/courses/{courseID}/lessons/{lessonID}/test/attempts`
    - `GET /api/teacher/courses/{courseID}/lessons/{lessonID}/test/analytics`
    - `GET /api/admin/courses/{courseID}/lessons/{lessonID}/test/analytics`
- Frontend (`React`):
  - API client (`frontend/src/app/utils/api.ts`) switched from client-only scoring to backend-driven test flow.
  - Added typed models for test settings, attempts, results, and analytics.
  - `LessonEditor.tsx`:
    - test settings UI (time/pass score/max attempts/random pool/shuffle/show answers).
    - additional question type `true_false`.
    - difficulty selector per question.
    - open-answer questions now use `correctText`.
  - `LessonViewer.tsx`:
    - start/submit server-side attempts.
    - history of attempts with scores.
    - student score card after submit.
    - teacher/admin analytics card with pass/fail, attempts used, and difficulty stats.
- Validation:
  - backend: `go test ./...` - OK
  - frontend: `npm run build` - OK
  - frontend: `npm run check:encoding` - FAIL (legacy mojibake remains in course lesson files; functional changes still build and run).
- 2026-05-11: ��������� ��������� ������: ����� ������� �� 300 �����, ����� ��������� �������� �� 100, ������� ���������� � �����, ��������� ������ +1 ������� ����� ���������� ������.

## 123) 2026-05-13 - Removed question difficulty from LMS tests
- Goal: fully remove the `difficulty` field from test questions.
- Backend changes:
  - `backend/internal/domain/models.go`: removed `Difficulty` from:
    - `LessonQuestion`
    - `LessonTestQuestionPublic`
    - `LessonTestQuestionAnalytics`
  - `backend/internal/service/field_limits.go`:
    - removed `MinQuestionDifficulty` / `MaxQuestionDifficulty`
    - removed difficulty validation in `validateLessonTestData`.
  - `backend/internal/service/course_service.go`:
    - removed difficulty normalization in attempt question builder
    - removed difficulty from public test question payload
    - removed difficulty aggregation in analytics.
- Frontend changes:
  - `frontend/src/app/utils/api.ts`:
    - removed `difficulty` from test interfaces and mappers
    - removed difficulty-related localized error pattern.
  - `frontend/src/app/components/courses/LessonEditor.tsx`:
    - removed difficulty from editable question model
    - removed difficulty select from test question form
    - removed difficulty from lesson payload.
  - `frontend/src/app/components/courses/LessonViewer.tsx`:
    - analytics block renamed to question stats without difficulty.
  - `frontend/src/app/utils/limits.ts`:
    - removed unused difficulty limits.
- Validation:
  - backend: `go test ./...` - OK
  - frontend: `npm run check:encoding` - OK
  - frontend: `npm run build` - OK

## 124) 2026-05-13 - Removed student extra test attempt button
- Frontend (`LessonViewer`): removed student UI for extra attempt (`+1 попытка`).
- Removed local state and dialog branches related to `forceExtraAttempt`.
- Test start confirmation now has a single standard flow: `Начать попытку теста`.
- Frontend API (`api.ts`): removed `forceExtraAttempt` parameter from `startLessonTest` request payload.
- Validation:
  - frontend: `npm run check:encoding` - OK
  - frontend: `npm run build` - OK

## 125) 2026-05-13 - Test retake policy and per-student reset
- Implemented new test setting `allowRetakeAfterPass` (default `false`).
- Student behavior:
  - If any submitted attempt is `passed` and `allowRetakeAfterPass=false`, starting a new attempt is blocked.
  - Backend returns `test already passed`; frontend shows localized message.
- Teacher/admin behavior:
  - Added per-student reset of test results for a specific lesson.
  - New endpoints:
    - `DELETE /api/teacher/courses/{courseID}/lessons/{lessonID}/test/students/{studentID}/reset`
    - `DELETE /api/admin/courses/{courseID}/lessons/{lessonID}/test/students/{studentID}/reset`
  - Reset removes student's attempts for that test lesson, removes lesson completion flag for this lesson, and recalculates enrollment progress.
- UI updates:
  - `LessonEditor`: checkbox `Разрешить повторное прохождение после успешной сдачи`.
  - `LessonViewer` (teacher/admin analytics): button `Сбросить результат` per student.
  - `LessonViewer` (student): start block reason shown when test already passed and retake is disabled.
- Validation:
  - backend: `go test ./...` - OK
  - frontend: `npm run check:encoding` - OK
  - frontend: `npm run build` - OK

## 126) 2026-05-13 - Admin permanent delete for removed courses
- Added hard-delete flow for admin from deleted courses block.
- Backend:
  - `CourseRepository` extended with `PermanentlyDeleteCourse(courseID)`.
  - Postgres repo implemented permanent delete:
    - deletes `enrollments` for course
    - deletes course row (dependent course data removed via FK cascades where configured).
  - New service method `PermanentlyDeleteByAdmin(courseID)` with guard:
    - permanent delete is allowed only when `course.status == rejected` (i.e. from deleted block).
  - New admin endpoint:
    - `DELETE /api/admin/courses/{id}/permanent`
- Frontend:
  - `api.ts`: added `permanentlyDeleteCourseAsAdmin(courseId)`.
  - Added error localization for backend guard: `course must be in deleted status`.
  - `AdminUserPage`: in `Удаленные курсы` block added button `Удалить окончательно` with confirmation.
- Validation:
  - backend: `go test ./...` - OK
  - frontend: `npm run check:encoding` - OK
  - frontend: `npm run build` - OK

## 127) 2026-05-13 - Protect creator admin role from created admins
- Added user ownership metadata:
  - `users.created_by_admin_id` (nullable FK to `users.id`) via migration.
  - Domain model: `User.CreatedByAdminID` (internal, not exposed in JSON).
- Admin-created users now store creator admin id:
  - `createUserByAdmin` passes current admin ID to `AdminService.CreateUser(...)`.
  - Postgres user insert now writes `created_by_admin_id`.
- Role-change protection:
  - In `changeRole` handler, before applying role update, current admin is loaded.
  - If target user id equals `currentUser.CreatedByAdminID`, request is blocked with:
    - `cannot change creator admin role`.
- Frontend localization:
  - Added translation for `cannot change creator admin role`.
- Validation:
  - backend: `go test ./...` - OK
  - frontend: `npm run check:encoding` - OK
  - frontend: `npm run build` - OK

## 128) 2026-05-13 - Fixed self-delete admin account bug
- Backend hard protection:
  - In `blockUser` handler, blocking own account is forbidden.
  - New guard: if `req.blocked=true` and target user equals current auth user -> `cannot block own user` (HTTP 403).
- Frontend UX protection:
  - `AdminDashboard`: delete action for current user is disabled and guarded in handler.
  - `AdminUserPage`: delete button is hidden when admin opens their own profile.
- Frontend localization:
  - Added translation for `cannot block own user`.
- Validation:
  - backend: `go test ./...` - OK
  - frontend: `npm run check:encoding` - OK
  - frontend: `npm run build` - OK

## 129) 2026-05-13 - Safe permanent user deletion (admin)
- Added hard-delete flow for users with safety rules:
  - available only via admin endpoint `DELETE /api/admin/users/{id}/permanent`
  - requires body confirmation `{"confirmation":"DELETE"}`
  - user must be in blocked/deleted state first
  - 30-day cooldown after soft delete (`users.blocked_at`) is enforced
  - deletion is blocked if dependencies exist:
    - active courses
    - deleted courses (must be permanently removed first)
    - active enrollments
    - pending teacher submissions
    - pending student submissions
    - dependent admins created by this user
  - extra protection:
    - cannot permanently delete own current admin account
    - cannot permanently delete creator-admin of current admin
- Backend:
  - migration updates:
    - `users.blocked_at TIMESTAMPTZ`
    - `admin_audit_logs` table with indexes
  - `UserRepository` extended with:
    - `GetPermanentDeleteConstraints`
    - `PermanentlyDeleteWithAudit`
  - `postgres/user_repo`:
    - users now read/write `blocked_at`
    - `SetBlocked` now sets/clears `blocked_at`
    - hard delete runs in transaction with audit log insert
  - `AdminService`:
    - new `PermanentlyDeleteUser(actorID, targetID)` with all guards/checks
  - `handler`:
    - new route `DELETE /api/admin/users/{id}/permanent`
- Frontend:
  - `api.ts`:
    - new `permanentlyDeleteUser(userId, confirmation)`
    - added localized errors for cooldown/dependency/confirmation cases
    - `User` model now supports `blockedAt`
  - `AdminDashboard`:
    - deleted-users tab now has two actions:
      - restore
      - permanent delete
    - permanent delete opens modal with typed confirmation (`DELETE`)
- Validation:
  - backend: `go test ./...` - OK
  - frontend: `npm run build` - OK

## 130) 2026-05-13 - Security hardening: headers, rate limit, audit logs
- Added global security headers middleware:
  - `X-Frame-Options: DENY`
  - `X-Content-Type-Options: nosniff`
  - `Referrer-Policy: strict-origin-when-cross-origin`
  - `Permissions-Policy: camera=(), microphone=(), geolocation=()`
  - strict `Content-Security-Policy` with allowed iframe/video domains (YouTube, VK, Rutube).
- Added global in-memory rate limiter middleware (IP + route rules):
  - `POST /api/auth/login`: 5/min with 15 min block
  - `POST /api/auth/register`: 3/10 min with 15 min block
  - `POST * /submission`: 10/min
  - admin user destructive routes (`PATCH/DELETE /api/admin/users/*`): 20/min
  - returns `429` + `Retry-After` + `rate limit exceeded`.
- Added full audit infrastructure:
  - new domain model `AdminAuditLog`
  - new repository interface `AuditRepository`
  - postgres implementation `backend/internal/repository/postgres/audit_repo.go`
  - service `backend/internal/service/audit_service.go`
  - handler-level helper logs `ip`, `user_agent`, `result` (`success/denied/error`).
- Added audit events for critical actions:
  - auth login success/failure
  - teacher publish/unpublish/delete course
  - teacher review submission approve/reject
  - admin restore/block/permanent delete user
  - admin change role
  - admin publish/unpublish/delete/permanent-delete course
- DB migration updated:
  - `users.blocked_at` maintained
  - `admin_audit_logs` extended with: `target_type`, `result`, `ip`, `user_agent`
  - compatibility `ALTER` queries for existing table.
- Frontend localization:
  - added translation for `rate limit exceeded`.
- Validation:
  - backend: `go test ./...` - OK
  - frontend: `npm run build` - OK
